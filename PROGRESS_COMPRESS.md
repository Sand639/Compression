...
- **WAV を3点改善し explosion.wav 268,956 -> 232,430 (-36,526!)、合計 ~1,209,808 -> ~1,173,145**:
  1. **ファイル別ステレオモード選択** (M/S, L/S, R/S, L/R を各々CM符号化し最小採用、先頭1バイトに保存):
     explosion.wav は mid/side が最適でなく -7,429。
  2. **残差をバイトプレーン分離→フルインターリーブ** (フレーム毎 mLo,mHi,sLo,sHi):
     **-28,913**。各16bit値の lo,hi 連続でCMの多バイト文脈が16bit残差を強力にモデル化。
     ※ lo,lo,hi,hi 配置は16bit分断で +10,918 と悪化 → 各16bit連続が鍵。
  3. **スパース文脈刻みを 4 (フレーム整列)**: -319。
- 教訓: 多バイト値はバイトプレーン分離せず連続配置するとCMの多バイト文脈が活きる。
- 失敗(revert): LPC次数20(+100) / exe APM4縮小apm4Sh=27(+228) / WAV subShift22(+117)。
