...
- explosion.wav 268,956 -> 231,677 (-37,279!)。合計 ~1,209,808 -> ~1,172,380。
- 教訓: 多バイト値はバイトプレーン分離せず連続配置するとCMの多バイト文脈が活きる。
  インターリーブ後の残差は構造的・定常的なので、より遅い床/細かいブロックが効く。
- 失敗(revert): LPC次数20/24 / exe APM4縮小(+228) / WAV subShift22/mixShift12/apmShift8 / WAV BS2048 /
  LMSカスケード(単段64/11が最良) / BMPカラー変換subtract-red(+7,470,greenが最適)。
