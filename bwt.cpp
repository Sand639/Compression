...
struct CMModel {
    ...
    CMModel(const CMProfile& prof = CM_PROF_SLOW)
              : TBITS(prof.tbits), TSIZE(1 << prof.tbits), TMASK((1 << prof.tbits) - 1),
                ...
                apm(16384 * 65), apm2(4096 * 65), apm3(2048 * 65), apm4(32768 * 65) {
    ...
    int predict() {
        ...
        // APM4: prf を cx[3]ハッシュ上位12bit+bitpos でさらに補正 (65点補間)
        apm4Ctx = static_cast<int>(((cx[3] * 0x9E3779B1u) >> 20) * 8 + bitpos);  // 32768文脈
        {
